...
class FlashMemory(MemoryRegion):
    ...
    def load_image(self, data: bytes) -> None:
        ...
        if len(data) > len(self._data):
            raise ValueError(
                f"Firmware {len(data)} bytes exceeds flash {len(self._data)} bytes"
            )
...
class BitBandRegion(MemoryRegion):
    ...
    def translate(self, alias_address: int) -> tuple[int, int]:
        ...
        if not self.target.contains(target_address):
            raise MemoryBoundsError(alias_address, 4, f"{self.name} -> invalid target")

        return target_address, bit_index

    def read(self, address: int, size: int) -> int:
        raise RuntimeError(
            "BitBand regions are not directly readable; use AddressSpace"
        )

    def write(self, address: int, size: int, value: int) -> None:
        raise RuntimeError(
            "BitBand regions are not directly writable; use AddressSpace"
        )
...
class PeripheralWindow(MemoryRegion):
    ...
    def read(self, address: int, size: int) -> int:
        raise RuntimeError(
            "PeripheralWindow.read() should not be called; use AddressSpace"
        )

    def write(self, address: int, size: int, value: int) -> None:
        raise RuntimeError(
            "PeripheralWindow.write() should not be called; use AddressSpace"
        )
