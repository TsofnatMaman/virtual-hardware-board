...
class UnicornEngine:
    ...
    def add_memory_hook(self, callback, begin: int, end: int) -> None:
        ...
        self.uc.hook_add(
            UC_HOOK_MEM_READ | UC_HOOK_MEM_WRITE, callback, begin=begin, end=end
        )
...
class CortexM(ICPU):
    ...
    def step(self) -> None:
        ...
        try:
            ...
            logger.error("CPU execution error at PC=0x%08X: %s", pc, exc)
            raise
    ...
    def _memory_hook(self, uc, access, address, size, value, _user_data):
        ...
        try:
            ...
            logger.error("MMIO access error at 0x%08X: %s", address, exc)
            raise
