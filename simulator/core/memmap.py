...
from simulator.core.exceptions import MemoryAccessError, MemoryAlignmentError, MemoryBoundsError
...
class AddressSpace:
    ...
    def read_block(self, address: int, size: int) -> bytes:
        ...
        if self.flash.contains(address) and self.flash.range.contains_range(address, size):
            return self.flash.read_block(address, size)
        if self.sram.contains(address) and self.sram.range.contains_range(address, size):
            return self.sram.read_block(address, size)
        raise MemoryAccessError(address, message="Block read not supported for this region")
    ...
    def _validate_access(self, address: int, size: int) -> None:
        ...
        if size not in (1, 2, 4):
            raise MemoryAccessError(address, message="Access size must be 1, 2, or 4 bytes")
    ...
    def _bitband_read(self, bitband: BitBandRegion, address: int) -> int:
        ...
        if bitband.target_is_peripheral:
            ...
            if not mapping:
                raise MemoryAccessError(target_addr, message="No peripheral under bitband")
