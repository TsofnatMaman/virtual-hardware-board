...
def test_stm32_board_wiring_and_step(monkeypatch):
    dummy_cpu = DummyCPU()
    monkeypatch.setattr(
        "simulator.stm32.board.create_cpu_for_address_space", lambda _addr: dummy_cpu
    )

    board = STM32F4Board()
    assert board.name == "STM32F4"
    ...
    board.step(3)
    assert dummy_cpu.ticks == [3]
    assert board.clock.cycle_count == 3

    # Ensure peripherals got interrupt controller
    periph = next(iter(board.peripherals.values()))
    assert getattr(periph, "_interrupt_controller", None) is board.interrupt_ctrl


def test_stm32_board_pin_mask_and_gpio_kind_errors(monkeypatch):
    dummy_cpu = DummyCPU()
    monkeypatch.setattr(
        "simulator.stm32.board.create_cpu_for_address_space", lambda _addr: dummy_cpu
    )
...
def test_stm32_board_read_write_and_reset(monkeypatch):
    dummy_cpu = DummyCPU()
    monkeypatch.setattr(
        "simulator.stm32.board.create_cpu_for_address_space", lambda _addr: dummy_cpu
    )
    ...
    # Mark a pending interrupt then reset
    periph = next(iter(board.peripherals.values()))
    board.interrupt_ctrl.notify(periph, vector=1)
